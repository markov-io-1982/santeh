...
$this->registerLinkTag(['rel' => 'canonical', 'href' => Url::canonical().'/'.$cur_category->slug]);
...
if($cur_category->parent_id != 0) {
	$parent = \frontend\models\Category::find()->select(['name_'.Yii::$app->language, 'slug'])->where(['id' => $cur_category->parent_id])->one();
	$name = 'name_'.Yii::$app->language;
	$this->params['breadcrumbs'][] = [
		'label' => $parent->$name,
...
$this->params['breadcrumbs'][] = $cur_category->$name;
...
<!--			--><?//= CategoryWidget::widget() ?>
		...
		<div class="col-md-9">
			...
			<div class="products-catalog">
				...
				<div class="product-catalog-item col-sm-4 col-lg-4 col-md-4">
					<div class="thumbnail" id="product-<?= $product->id ?>">
						<a href="<?= Url::to(['products/' . $cur_category->slug.'/'.$product->slug]) ?>">
					    	<?= Html::img('@web/' . $product->img, ['class' => '', 'title' => '', 'style' => 'width:320px; height:250px;']) ?>
					    </a>
					    <div class="caption">
					        <h4 class="title">
					        	<?= Html::a($product->$name, ['products/' . $cur_category->slug.'/'.$product->slug], ['class' => '', 'style' => '']); ?>
					        </h4>
            ...
            <div style="padding: 20px 0;"><?= $cur_category->$description ?></div>
